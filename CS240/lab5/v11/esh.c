...
int main() {
	...
	char quick[MAXSIZE] = " "; //for quick function. Holds specified string
	...
	while(1) {
		...
		int quickUsed = 0; //zero means not used, one means used
		...
		quickUsed = quickSet(str, quick, strLen);
		...
		if ((cprtUsed == 0) && (lckUsed == 0) && (countUsed == 0) && (quickUsed == 0)) {
			system(str);
		}
