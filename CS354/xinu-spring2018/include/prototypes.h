...
/* Addtions for lab5 */
extern syscall sendblk(pid32, umsg32); //Mimics send() if the 1 word buffer is empty, otherwise block indefinitely and add to queue
extern qid16 getNewQueue(void); //Used to initialize the queue for blocking processes
extern pid32 blkEnqueue(pid32, qid16); //Blocking process enqueue
extern pid32 blkDequeue(qid16); //Blocking process dequeue
extern pid32 blkGetFirst(qid16); //Get first item in queue and adjust pointers
extern pid32 blkGetLast(qid16); //Get last item in queue and adjust pointers
void receiveMessage(void);
void sendMessage(pid32, umsg32);
extern syscall cbreg( int (*func) (void) );
