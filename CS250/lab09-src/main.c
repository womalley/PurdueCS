...
int write_buf_size; //size of output buffer in use at the current time
...
int read_buf_size; //size of input buffer
int read_count; //counter for number of bytes read thus far
...
void myputc(char ch) {
        ...
        if (write_buf_size == loc) {
                write (fd_write, &write_buf[0], write_buf_size + 1);
                wp = &write_buf[0];
        }
}
...
void mywriteflush(void) {
	...
	if (loc >= 1) {
                ...
                write (fd_write, &write_buf[0], loc);
        }
