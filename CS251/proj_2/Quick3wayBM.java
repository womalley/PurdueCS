...
*	method depending on what the value of "n" is. Prints sorting methods and
	...
	public static void sort (Comparable[] a, int lo, int hi) {
		...
		*/
		if ((hi - lo) < 8) { //cut off point, compute if 8 or less
			...
			int loCopy = 0; //set loIndex to loVal for manipulation in inner loop

			//Insertion sort method
			for (loIndex = lo; loIndex <= hi; loIndex++) {
				for (loCopy = loIndex; loCopy > lo; loCopy--) {
					if (a[loCopy].compareTo(a[loCopy - 1]) < 0) {
						swapper(a, loCopy, loCopy - 1);
					}
					else {
